import { Timer, ClipboardList, DollarSign, GraduationCap, Users, BookOpen, UserCog, Settings, ChevronRight, X } from 'lucide-react'
...
const menuItems = [
  ...
  { icon: Settings, label: 'Sozlamalar', path: '#' },
]
  ...
  return (
    ...
    `}>
      ...
      <div className="px-6 py-4">
        <h1 className="text-[#0ea5e9] text-3xl font-semibold">
          Soff Crm
        </h1>
      </div>
      ...
      <nav className="flex-1 px-2 py-2">
        {menuItems.map((item) => (
          <div
            key={item.label}
            href={item.path}
            className="flex items-center justify-between px-4 py-2 mt-1 rounded-lg text-gray-300 hover:bg-white/10 hover:text-[#0ea5e9] group transition-colors"
          >
            <div className="flex items-center gap-3">
